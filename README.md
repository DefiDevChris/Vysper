...
| Shortcut | Action |
|----------|--------|
| `Cmd+Shift+\` | Clear Session Memory (Blank Context) |
| `Cmd+Shift+D` | Cycle/Lock Display (Dual Monitor Stealth) |

### Auto-Typing
| Shortcut | Action |
|----------|--------|
| `F8` | Start Real-time Auto-typing |
...
### Stealth Technology
- **Wayland Content Protection** - Native isolation on modern Linux desktops
...
- **Dual-Monitor Locking** - Lock the assistant to a non-shared screen
...
## 🐧 Linux & Wayland Stealth Setup

Vysper is optimized for Fedora/GNOME/Wayland. To ensure 100% absence from screen sharing:

1. **Native Wayland Mode**: Always run with `npm start`. This enables the **Ozone/Wayland** backend which supports compositor-level layer exclusion.
2. **Dual-Monitor Setup (Recommended)**: 
   - Use `Ctrl+Shift+D` to move Vysper to your laptop screen.
   - Share only your external monitor.
   - This provides hardware-level isolation.
3. **GNOME Settings**: Ensure you are using the standard "GNOME" (Wayland) session, as X11 does not support the same level of capture exclusion.

## Important Technical Requirements
- **Node.js** 18+
- **NVIDIA NIM API Key** (for Vision & LLM)
- **Tesseract OCR** (`brew install tesseract` or `sudo dnf install tesseract`)
- **Audio Tool** (`brew install sox` or `sudo dnf install sox`)
