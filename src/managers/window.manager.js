...
class WindowManager {
  constructor() {
    ...
    this.isInteractive = process.platform === 'linux'; // Default to interactive on Linux to avoid being stuck
  ...
  async createWindow(type, showOnCreate = false) {
    ...
    const baseOptions = {
      ...
      // Linux PipeWire exclusion - prevents window from appearing in screen sharing
      // In native Wayland mode, 'notification' type windows are the most reliably 
      // excluded layer from the desktop capture stream.
  ...
  applyStealthMeasures(window, type) {
    ...
    // On Linux/Wayland, this can cause the ScreenCast portal to reset if applied to the session owner
    try {
      window.setContentProtection(true);
      // Set slight opacity to force compositor-level layer processing
      // which improves the effectiveness of content protection on some Linux desktops
      window.setOpacity(0.99);
    } catch (error) {
      logger.debug('Content protection or opacity setting failed', { error: error.message });
    }

    // Linux-specific: Additional PipeWire exclusion via window type 'popup_menu'
    if (process.platform === 'linux') {
      logger.info('Linux stealth mode: PipeWire/X11 exclusion enabled via content protection + window type popup_menu');
    }
  ...
  setupScreenSharingDetection() {
    // Screen sharing detection is problematic on Wayland as it triggers portal prompts
    if (process.platform === 'linux') {
      logger.info('Screen sharing detection disabled on Linux to prevent portal issues');
      return;
    }
  ...
  // New: Cycle windows between available displays
  cycleDisplay() {
    ...
    if (this.currentDisplay) {
      const currentIndex = displays.findIndex(d => d.id === this.currentDisplay.id);
      nextIndex = (currentIndex + 1) % displays.length;
    }

    const nextDisplay = displays[nextIndex];
    this.lockedDisplayId = nextDisplay.id;
    this.currentDisplay = nextDisplay;
    
    logger.info('Cycling to next display and locking', { 
      displayId: nextDisplay.id, 
      isPrimary: nextDisplay.id === screen.getPrimaryDisplay().id 
    });
  ...
  // New: Lock to a specific display
  lockToDisplay(displayId) {
    const displays = screen.getAllDisplays();
    const target = displays.find(d => d.id === displayId || d.id.toString() === displayId.toString());
    
    if (target) {
      this.lockedDisplayId = target.id;
      this.currentDisplay = target;
      logger.info('Locked windows to display', { displayId: target.id });
      this.moveWindowsToActiveScreen();
      return true;
    }
    return false;
  ...
  // New: Unlock from specific display (resume following mouse)
  unlockDisplay() {
    this.lockedDisplayId = null;
    logger.info('Unlocked display (following mouse again)');
    this.trackActiveScreen();
  }

  trackActiveScreen() {
    // If locked to a display, don't follow the mouse
    if (this.lockedDisplayId) return;
